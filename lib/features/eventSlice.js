...
export const fetchAllEvents = createAsyncThunk(
  "/api/event/get/all",
  async ({ page, limit, searchText, city, dateFilter }, { rejectWithValue }) => {
    try {
      const params = new URLSearchParams({
        ...
        search: searchText || "",
        city: city || "all",
        date: dateFilter || "any", // "any", "today", "tomorrow", "week", "month"
      });

      const response = await fetch(`/api/event/get/all?${params.toString()}`);
      const data = await response.json();
